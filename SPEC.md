...
| `Starkit describe` | Asks the **Scripts** what they are through the same decoder the bar reads them with, refreshes `manifests.json`, and prints one line per **Script**. The contract between the two halves: `entry.gleam` writes the **Manifests** and nothing generates its spelling from `Manifest`'s, so a field renamed on one side and not the other fails here. Run in CI and by `just test` for exactly that reason — every other test in the repo decodes a string a person typed. |
...
| `Starkit edit <keyword>` | Opens `src/scripts/<keyword>.gleam` in the `editor` named in `starkit.toml`, or in Zed when the file names none, which is what ⌥↩ (or ⌃O) on a selected **Script** does in the bar. **Refuses** when the file is not there instead of writing a template over the question. |
...
├── starkit.toml                  # optional: bun and gleam paths, and the editor ⌥↩ opens; absent by default

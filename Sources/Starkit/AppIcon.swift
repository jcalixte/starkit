...
/// Starkit's icon in Finder, drawn rather than stored, from the same `Carambola` path as the bar's
/// mark. Written as an `.iconset` of PNGs because `iconutil` is the only thing that writes `.icns`,
/// and it reads a directory of PNGs named its way.
enum AppIcon {
    ...
    /// bar's mark uses.
    ///
    /// The smallest size decides both: Finder's list view asks for 16 pixels, where the mark's stroke
    /// renders as a bruise and solid ink is still recognisably a star.
    private static let fruit: CGFloat = 0.74

    /// What `iconutil` demands, under the names it demands them under. Two pairs come out identical
    ...
    /// One PNG, `pixels` on a side. sRGB is named rather than left to the device, for the reason
    /// `Palette` gives its hexes in sRGB: a bitmap tagged with the drawing Mac's display space would
    /// put a different periwinkle in the file depending on which Mac ran the build.
    ...
    /// The plate, then the fruit on it, into whichever context is current. A circular corner, not the
    /// continuous one the bar's chip has: AppKit exposes that curve on `CALayer` only.
